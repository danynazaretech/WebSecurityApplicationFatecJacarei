...
app.post('/change-password-vulnerable', (req, res) => {
    ...
    console.log(`[VULNERÁVEL] Tentativa de mudança de senha para: ${new_password}`);
    if (new_password) {
        userPassword = new_password;
        console.log(`[VULNERÁVEL] Senha alterada com sucesso para: ${userPassword}`);
...
app.post('/change-password-protected', csrfProtection, (req, res) => {
    ...
    console.log(`[PROTEGIDO] Tentativa de mudança de senha para: ${new_password}`);
    ...
    if (new_password) {
        ...
        return res.redirect('/protected-form?status=password_not_changed_protected');
    }
});
...
app.use((err: any, req: any, res: any, next: any) => {
    if (err.code === 'EBADCSRFTOKEN') {
        ...
        res.status(403).send('Erro CSRF: Token de requisição inválido. Sua sessão pode ter expirado ou a requisição é maliciosa.');
    } else {
        next(err);
    }
});
